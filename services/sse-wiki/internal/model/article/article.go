...
type ArticleVersion struct {
	ID            uint      `gorm:"primaryKey" json:"id"`
	ArticleID     uint      `gorm:"not null;index:idx_article_version" json:"article_id"`
	// 版本号，在article_id下递增 (1, 2, 3...)
	VersionNumber int       `gorm:"not null;index:idx_article_version" json:"version_number"`
	// Markdown原文，全量存储
	...
	// 版本状态: draft(草稿/未发布), published(已发布), archived(已归档)
